...
parser = argparse.ArgumentParser(description='Encoder-Decoder Butterfly Network')
...
parser.add_argument('--dataset', default='MNIST', type=str,
                    help='dataset (default: (MNIST), other options are Faces, Gaussian1, Gaussian2')
...
if args_data == 'MNIST':
    ...
    print ('Calculating 1024 ort vectors')

    ort_vecs_1024 = get_ort_vectors(1024)
    ...
    print ()
    print ('Running BF Encoder on various values of k')
    print ()
    criterion = nn.MSELoss()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    d = 1024
    ...
    weights, masks = get_weights_and_masks(int(np.log2(1024)))
    for k in ks:
        print ('k = {}'.format(k))
        print()
        model = SparseAutoEncoderMNIST(k=k,batch_size = 512)
        optimizer = torch.optim.Adam(model.parameters())
        model.to(device)
        sparse_train(model,optimizer,args_epochs,k,train_loader) 



if args_data == 'Faces':
    
    print ('Calculating 4096 ort vectors')

    ort_vecs_4096 = get_ort_vectors(4096)
    ...
    print ('Using {} device'.format(device))
    print ()
    dataset = getFacesData()
    train_loader = getTorchDataLoader(dataset, batch_size=512)
    weights, masks = get_weights_and_masks(int(np.log2(4096)))

    for k in ks:
        print ('k = {}'.format(k))
        print()
        model = SparseAutoEncoderFaces(k=k,batch_size = 400)
        optimizer = torch.optim.Adam(model.parameters())
        model.to(device)
        sparse_train(model,optimizer,args_epochs,k,train_loader) 


if args_data == 'Gaussian1':
    ...
    print ('Using {} device'.format(device))
    print ()
    dataset = getLowRankData(d,d,rank)
    train_loader = getTorchDataLoader(dataset, batch_size=512)
    weights, masks = get_weights_and_masks(int(np.log2(1024)))
    ...
    for k in ks:
        print ('k = {}'.format(k+1))
        print()
        model = SparseAutoEncoderMNIST(k=k+1,batch_size = 512)
        optimizer = torch.optim.Adam(model.parameters())
        model.to(device)
        sparse_train(model,optimizer,args_epochs,k+1,train_loader) 


if args_data == 'Gaussian2':
